...
class BadRequest extends ApiError {
	constructor(msg, code, detail) {
		super(400, msg, code, detail);
	}
}

class NotFound extends ApiError {
	constructor(msg, code, detail) {
		super(404, msg, code, detail);
	}
}

class Conflict extends ApiError {
	constructor(msg, code, detail) {
		super(409, msg, code, detail);
	}
}
...
express.application.allx = function (path, callback) {
	this.all(path, async (req, res, next) => {
		try {
			const data = await callback(req, res, next);
			res.json(data);
		} catch (ex) {
